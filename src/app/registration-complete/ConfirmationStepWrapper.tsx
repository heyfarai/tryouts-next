...
const ConfirmationStepWrapper: React.FC<ConfirmationStepWrapperProps> = ({ sessionId, confirmation }) => {
  const [receiptUrl, setReceiptUrl] = useState<string | undefined>(undefined);
  useEffect(() => {
    async function fetchReceiptUrl() {
      if (!sessionId) return;
      // Get the Stripe Checkout Session, expand payment_intent
      const res = await fetch(`/api/checkout-session-info?session_id=${encodeURIComponent(sessionId)}`);
      ...
      if (data.confirmation && data.confirmation.paymentIntentId) {
        ...
        const receiptRes = await fetch(`/api/stripe-receipt-url?payment_intent_id=${encodeURIComponent(data.confirmation.paymentIntentId)}`);
